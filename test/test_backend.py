...
from collection.toolkit.datetime import (InvalidDateError,
                                          datetime_utcnow,
                                          str_to_datetime)
from collection.backends.core import __version__
from collection.archive import Archive, ArchiveManager
...
from collection.errors import ArchiveError, BackendError, BackendCommandArgumentParserError
...
SUMMARY_LOG_REPORT = """INFO:collection.backend:Summary of results
...
class TestBackendBlacklist(unittest.TestCase):
    ...
    def test_fetch_blacklist(self):
        ...
        with self.assertLogs(backend_logger, level='INFO') as cm:
            items = [item for item in backend.fetch(category=MockedBackendBlacklist.DEFAULT_CATEGORY)]
            self.assertEqual(cm.output[0], 'WARNING:collection.backend:Skipping blacklisted item item 1')
...
class TestBackendCommand(unittest.TestCase):
    ...
    def test_summary_logging(self):
        ...
        with self.assertLogs('collection.backend', level='INFO') as cm:
            cmd = MockedBackendCommand(*args)
            cmd.run()
            cmd.outfile.close()
...
class TestMetadata(unittest.TestCase):
    ...
    def test_metadata(self):
        ...
        for x in range(5):
            ...
            self.assertEqual(item['collection_version'], __version__)
