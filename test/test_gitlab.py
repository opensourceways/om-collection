...
from collection.toolkit.datetime import datetime_utcnow
from collection.backend import BackendCommandArgumentParser
from collection.errors import (BackendError,
                             HttpClientError,
                             RateLimitError)
from collection.utils import DEFAULT_DATETIME
from collection.backends.core.gitlab import (logger,
                                           GitLab,
                                           GitLabCommand,
                                           GitLabClient,
                                           CATEGORY_ISSUE,
...
class TestGitLabBackend(unittest.TestCase):
    ...
    def test_fetch_issues_blacklisted(self):
        ...
        with self.assertLogs(level='WARNING') as cm:
            issues = [issues for issues in gitlab.fetch()]
            self.assertEqual(cm.output[0], 'WARNING:collection.backend:Skipping blacklisted item iid 1')
            self.assertEqual(cm.output[1], 'WARNING:collection.backend:Skipping blacklisted item iid 2')
            self.assertEqual(cm.output[2], 'WARNING:collection.backend:Skipping blacklisted item iid 3')
...
class TestGitLabClient(unittest.TestCase):
    ...
    def test_initialization_http_error(self):
        ...
        with self.assertLogs(logger, level='WARNING') as cm:
            GitLabClient("fdroid", "fdroiddata", "your-token")
            self.assertEqual(cm.output[0], 'WARNING:collection.backends.core.gitlab:Rate limit not initialized: 400 '
