...
from collection.backend import BackendCommandArgumentParser, uuid
from collection.errors import RepositoryError
from collection.utils import DEFAULT_DATETIME, DEFAULT_LAST_DATETIME
from collection.backends.core.git import (EmptyRepositoryError,
                                        Git,
                                        GitCommand,
                                        GitParser,
                                        GitRepository)
...
class TestGitBackend(TestCaseGit):
    ...
    @classmethod
    def setUpClass(cls):
        cls.tmp_path = tempfile.mkdtemp(prefix='collection_')
    ...
    def test_git_cr_error(self):
        ...
        "collection.errors.ParseError: commit expected on line 10"
...
class TestGitCommand(TestCaseGit):
    ...
    def setUp(self):
        super().setUp()
        self.tmp_path = tempfile.mkdtemp(prefix='collection_')
...
class TestGitParser(TestCaseGit):
    ...
    def test_stats_pattern(self):
        ...
        s = "8\t7\tcollection/backends/gerrit.py"
        m = pattern.match(s)
        self.assertEqual(m.group('added'), "8")
        self.assertEqual(m.group('removed'), "7")
        self.assertEqual(m.group('file'), "collection/backends/gerrit.py")
...
class TestGitRepository(TestCaseGit):
    ...
    def test_count_objects_invalid_output(self):
        ...
        with unittest.mock.patch('collection.backends.core.git.GitRepository._exec') as mock_exec:
