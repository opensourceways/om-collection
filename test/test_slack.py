...
from collection.backends.core.slack import (logger,
                                          Slack,
                                          SlackClient,
                                          SlackClientError,
                                          SlackCommand)
...
class TestSlackBackend(unittest.TestCase):
    ...
    @httpretty.activate
    @unittest.mock.patch('collection.backends.core.slack.datetime_utcnow')
    ...
    def test_fetch_archived_channel(self, mock_utcnow):
        ...
        with self.assertLogs(logger, level='WARNING') as cm:
            messages = [msg for msg in slack.fetch(from_date=None)]
            self.assertEqual(cm.output[0], 'WARNING:collection.backends.core.slack:'
