...
class GiteeScore(object):
    def __init__(self, config=None):
        ...
        self.giteeClient = GiteeClient(
            owner=self.owner, repository=self.repository, token=self.access_token)
        ...
        self.bug_fragemnt_authorization = config.get(
            'bug_fragemnt_authorization')
    ...
    def run(self, from_date):
        ...
        self.bugFragment_email_map = self.assemble_email_map(
            bug_questionnaire_list)

        self.robot_user_list = [robot_user.strip()
                                for robot_user in self.robot_user_login.split(',')]
    ...
    def get_score_admins(self):
        ...
        if yaml_response.status_code != 200:
            ...
            return None
        score_admins = [username.strip()
                        for username in yaml_response.text.split('\n')[1:]]
        return score_admins
    ...
    def process_issue_score(self):
        ...
        for issue_brief in issue_brief_list:
            ...
            issue_comment_list = self.get_comments_by_issue_number(
                issue_number)
            author_username, score = self.parse_comment_list(
                issue_comment_list)
            print(
            ...
            action = common.getSingleAction(
                self.index_name, issue_number, content_body)
            actions += action
    ...
    def process_pull_score(self):
        ...
        for pull_brief in pull_brief_list:
            ...
            content_body = {}
            content_body['scoring_admin'] = score_admin
            content_body['score'] = pull_score
            content_body['pull_number'] = pull_number
            content_body['created_at'] = pull_created_at
            content_body['user_login'] = user_login
            content_body['data_type'] = 'pull_request'
            content_body['is_gitee_pull_request'] = 1
            action = common.getSingleAction(
                self.index_name, pull_number, content_body)
            actions += action
        self.esClient.safe_put_bulk(actions)
    ...
    def get_repo_issue_content_list(self):
        ...
        for page_issue in issue_generator:
            ...
            for single_issue_body in page_issue_list:
                ...
                version_num, folder_name, bug_fragment = self.parse_single_issue_body(
                    single_issue_body)
                ...
                page_issue_bodies.append(
                    (issue_number, user_login, created_at, version_num, folder_name, email))
    ...
    def get_repo_pull_content_list(self):
        ...
        pull_generator = self.giteeClient.pulls(
            state='all', once_update_num_of_pr=0)
        ...
        for single_pull_body in whole_pull_list:
            ...
            comments_generator = self.giteeClient.pull_review_comments(
                pull_number)
            score_admin, pull_score = self.parse_pull_comments(
                comments_generator)

            if not pull_score:
                continue

            total_pull_bodies.append(
                (pull_number, user_login, created_at, pull_score, score_admin))
    ...
    def get_comments_by_issue_number(self, issue_number):
        ...
        issue_comment_text = [
            issue_comment for issue_comment in issue_comment_generator][0]
    ...
    def parse_single_issue_body(self, single_issue_body):
        ...
        issue_number = single_issue_body.get('number')
        try:
            content_split_list = single_issue_body.get(
                "body").replace('\r', '').split('【')
            doc_link = content_split_list[1].split(
                'http')[1].split('\n')[0].strip()
            ...
            print(
                f'Issue number: {issue_number} failed to parse single_issue body in function: {sys._getframe().f_code.co_name}\n Error is: {exp.__repr__}')
    ...
    def assemble_email_map(self, bug_questionnaire_list):
        ...
        for bug_questionnaire in bug_questionnaire_list:
            ...
            bugDocFragment = bug_questionnaire.get(
                '_source').get('bugDocFragment')
