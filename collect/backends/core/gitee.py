...
import requests
from ...toolkit.datetime import (datetime_to_utc,
...
class Gitee(Backend):
    """Gitee backend for Collection.

    This class allows the fetch the issues stored in Gitee repostory.
    ```
    Gitee(
        owner='chaoss', repository='collection-gitee',
    ...
    def __get_pull_review_comments(self, pr_number):
        ...
        try:
            ...
            for raw_comments in group_comments:
                comment = self.__get_pull_review_comment_data(raw_comments)
    ...
    def __get_pull_review_comment_data(self, raw_comments):
        ...
        for comment in json.loads(raw_comments):
            ...
            if not user:
                logger.warning("Missing user info for %s:%s" % (comment['url'], comment_id))
